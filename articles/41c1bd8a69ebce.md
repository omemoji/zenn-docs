...
Vue.js 単体では不可能だった SSR に対応していることも特筆すべき特徴である。ただ、「創作物紹介」では動的にコンテンツを表示する必要はないので、SSG でページ生成を行っている。
...
なお、なぜかディレクトリ名を大文字にしたとき上手くいかなかったことがあったので、ディレクトリやファイルは小文字で命名した方が良いと思う。
...
その上で、現在かなり雑になっている部分(json で動的ルーティングを管理しているという実装手法や Nuxt Content に対する理解など)を見直そうと思う。
また、
